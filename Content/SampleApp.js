...
(function()
{
 var Global=this,Runtime=this.IntelliFactory.Runtime,jQuery,List,UI,Next,Doc,AttrProxy,T,SampleApp,Client,Arrays,PostModel,Key,ListModel,amplify,Var,alert,console;
 Runtime.Define(Global,{
  SampleApp:{
   Client:{
    ...
    PubSubDoc:Runtime.Field(function()
    {
     ...
     arg201=List.ofArray([Doc.TextNode("Create new subscription and invoke an event to be handled")]);
     ...
     {
      var value;
      value=amplify.publish.apply(amplify,["tryPubSub","Hello WebSharper!"]);
      return;
     };
     arg207=function()
     {
     ...
     arg204=List.ofArray([Doc.Button("Subsribe on message",Runtime.New(T,{
      $:0
     }),arg205),Doc.Button("Publish message",Runtime.New(T,{
      $:0
    ...
    RequestDoc:Runtime.Field(function()
    {
     ...
     ats=List.ofArray([AttrProxy.Create("class","panel panel-default")]);
     arg20=List.ofArray([Doc.TextNode(" Request")]);
     ats1=List.ofArray([AttrProxy.Create("class","panel-body")]);
     arg201=List.ofArray([Doc.TextNode(" Get single post by Id ")]);
     arg202=Runtime.New(T,{
      $:0
     });
     arg203=List.ofArray([Doc.TextNode("Demo:")]);
     ...
     arg207=List.ofArray([Doc.TextNode("Get all posts (with cache enabled)")]);
     arg208=Runtime.New(T,{
      $:0
     });
     arg209=List.ofArray([Doc.TextNode("Demo:")]);
     arg20b=function()
     {
      var reqs;
      reqs={
      ...
      reqs.success=function(data)
      {
       ...
       count=data.length;
       return alert("Found: "+Global.String(count)+" posts\nCheck console log for more details.");
      };
      return amplify.request(reqs);
     };
     arg20a=List.ofArray([Doc.Button("Get All",Runtime.New(T,{
      $:0
     }),arg20b)]);
     arg20c=Runtime.New(T,{
      $:0
     });
     arg20d=List.ofArray([Doc.TextNode("Create new posts")]);
     arg20e=Runtime.New(T,{
      $:0
     });
     arg20f=List.ofArray([Doc.TextNode("Demo:")]);
     attrs=Runtime.New(T,{
      $:0
     });
    ...
    StoreDoc:Runtime.Field(function()
    {
     ...
     arg205=function()
     {
      return amplify.store("tryStore","secret data");
     };
     arg206=function()
     {
      return alert(amplify.store("tryStore"));
     };
     arg207=function()
     {
      var value;
     ...
     arg204=List.ofArray([Doc.Button("Store",Runtime.New(T,{
      $:0
     }),arg205),Doc.Button("Retrive",Runtime.New(T,{
      $:0
    ...
    defineGetAll:function()
    {
     ...
     settings.dataType="json";
     settings.cache=true;
     return amplify.request.define("getAllPosts","ajax",settings);
    },
    defineGetById:function()
    {
     var settings;
     settings={};
     settings.url="https://jsonplaceholder.typicode.com/posts/{id}";
     settings.type="GET";
     settings.dataType="json";
     return amplify.request.define("getPostById","ajax",settings);
    },
    subscribeFn:function(data)
    {
     return alert(data);
    }
   }
 ...
 Runtime.OnInit(function()
 {
  ...
  AttrProxy=Runtime.Safe(Next.AttrProxy);
  T=Runtime.Safe(List.T);
  SampleApp=Runtime.Safe(Global.SampleApp);
  Client=Runtime.Safe(SampleApp.Client);
  Arrays=Runtime.Safe(Global.WebSharper.Arrays);
